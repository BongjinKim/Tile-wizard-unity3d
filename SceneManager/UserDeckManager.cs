...
public class UserDeckManager : MonoBehaviour {
	...
	/*
	 * 타일을 클릭하면 밑에 작은 타일칸이 생김
	 */
	GameObject MakeTileInfoDeck(int idx) {
        //Debug.Log(currentCountInDeck);
	...
	public void DeleteSelectedTile(GameObject sObj){
		...
		//타일프리셋지우기
		//Debug.Log ("지우기전 타일 번호 : "+tileIndex);
		...
		//Debug.Log("currentCountInDeck : " + currentCountInDeck);
		...
		if(sObj.name.Equals(tileIndex + "_Info") && number==false){
			...
			//Debug.Log(characterInfo.selectedTileDeck.tilesInDeck);
			Destroy(sObj); //오브젝트 삭제
            countingElementNum();
            convertElementNum();
           
			
			//Debug.Log (currentIdxInDeck);
			/*
			int idx = tileImgCountInDeck-1;
			if (idx <= 6) {
                DeckPanel.GetComponent<RectTransform>().sizeDelta = new Vector2(1260, 210);
			}else{
				float width = 1260 + 190*(idx-7);//tileImgCountInDeck이 지우기전의 값이라서 -1을 더 해줌
                DeckPanel.GetComponent<RectTransform>().sizeDelta = new Vector2(width, 210);
			}*/
			...
			if(tileIndex != tileImgCountInDeck){
				//Debug.Log("tileImgCountInDeck : " + tileImgCountInDeck);
	...
	public void MakeSelectedTile(GameObject sObj){
		//Debug.Log(sObj.name + "눌림");
		//Debug.Log("총 가질 수 있는 카드 개수 : " + InstantPresetObject.Count);
        ...
        if (isCardEmpty == false)
        {
            ...
            { // 데이터상의 개수와 진짜 개수가 같지 않으면
                //Debug.Log ("here");
                //Debug.Log ("currentIdxInDeck: " + currentIdxInDeck);
                //Debug.Log ("presetManager.tileImgInDeck.Count: " + presetManager.tileImgInDeck.Count);
                bool number = false;
    ...
    public void submitDeck()
    {
        if (getTotalNum() == 25)
        {
            ...
            Debug.Log(cnt);
            //Debug.Log(GameData.pc.tileDeckListOwnedByCharacter[GameData.pc.selectedTileDeckIndex].tilesInDeck[0]);
            SaveLoad.Save();
            Application.LoadLevel("4-1. INVENTORY");
        }
        else {
	...
	void LoadDeck(int num){
        //num = GameData.pc.selectedTileDeckIndex;
        ...
        {
            //Debug.Log(GameData.pc.tileDeckListOwnedByCharacter[num].inDeckData.Count);
